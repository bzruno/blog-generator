...
class StaticSiteGenerator:
    ...
    def _template(self, name):
        if name not in self.templates:
            if not (self.layouts_dir / name).exists():
                if name != "single.html": name = "single.html"
                else: sys.exit(f"Template {name} não encontrado")
            self.templates[name] = self.env.get_template(name)
        return self.templates[name]
    
    def _process_file(self, filepath, is_article=True):
        try: content = filepath.read_text(encoding="utf-8", errors="replace")
        except: return {}
        ...
        if part is None and series:
            if match := re.match(r'(\d+)-.*', filepath.stem): part = int(match.group(1))
        ...
        for dir_path in [self.content_dir / "articles", self.content_dir / "series"]:
            if dir_path.exists():
                for filepath in dir_path.rglob("*.md"):
                    if page := self._process_file(filepath, True): self.pages.append(page)
        
        for filepath in self.content_dir.glob("*.md"):
            if filepath.name != "_index.md":
                if page := self._process_file(filepath, False): self.pages.append(page)
    ...
    def generate(self):
        ...
        self._load_content()
        if not self.index_page: return print("Erro: _index.md não encontrado")
        self._copy_static()
        ...
        content = self.processor.process_shortcodes(self.index_page["content"], all_categories, self.pages)
        html_content, _ = self.processor.enhance_html(content, all_categories, self.pages)
        artlist = self.processor.shortcodes.get("artlist", lambda *args: "")(all_categories, articles)
        
        self._write(self.output_dir / "index.html", template.render(title=self.index_page["title"], content=html_content, artlist=artlist))
        ...
        for shortcode, output, tpl, title, data in special_pages:
            if shortcode in self.processor.shortcodes:
                ...
                self._write(self.output_dir / output / "index.html", self._template(tpl).render(title=title, **{shortcode: content}))
        ...
        if all_categories and "category" in self.processor.shortcodes:
            ...
            self._write(self.output_dir / "categorias" / "index.html", 
            ...
            if "artlist" in self.processor.shortcodes:
                for category in all_categories:
                    ...
                    if cat_posts:
                        ...
                        self._write(self.output_dir / "categorias" / Utils.normalize(category) / "index.html",
                                   self._template("categoria.html").render(title=category, category_normalized=Utils.normalize(category), artlist=content))

        # Séries
        series_list = sorted(set(p["series"] for p in articles if p["series"]))
        if series_list and "artlist" in self.processor.shortcodes:
            for serie in series_list:
                series_posts = sorted([p for p in articles if p["series"] == serie], key=lambda x: (x.get("part", 0), x["timestamp"]))
                if series_posts:
                    content = self.processor.shortcodes["artlist"]([], series_posts)
                    self._write(self.output_dir / "series" / Utils.normalize(serie) / "index.html",
    ...
    def serve(self):
        ...
        for path in [self.content_dir, self.layouts_dir, self.root/"static", self.root/"images", self.root/"videos"]:
            if path.exists(): observer.schedule(handler, path, recursive=True)
